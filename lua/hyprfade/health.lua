...
function M.check()
	...
	health.info(
		string.format(
			"term_names (config): %s",
			vim.inspect(
				vim.g.hyprfade_opts and vim.g.hyprfade_opts.term_names or { "ghostty", "kitty", "alacritty", "foot", "wezterm" }
			)
		)
	)
	...
	if not hyprland_sig or hyprland_sig == "" then
		health.warn(
			...
			{ "Hyprland may not be running", "Health checks beyond `hyprctl` availability will be limited" }
		)
	else
