...
function M.check()
  ...
  if not hyprland_sig or hyprland_sig == "" then
    health.warn("`$HYPRLAND_INSTANCE_SIGNATURE` is not set", { "Hyprland may not be running", "Health checks beyond `hyprctl` availability will be limited" })
  else
    local instance_dir = "/tmp/hypr/" .. hyprland_sig
    if vim.fn.isdirectory(instance_dir) == 1 then
      health.ok("Hyprland instance detected")
    else
      health.warn(string.format("Hyprland instance directory (%s) not found", instance_dir), { "Health checks beyond `hyprctl` availability will be limited" })
    end
  end

  local pid, name = find_terminal_pid({ "kitty", "alacritty", "foot", "wezterm" })
  if pid then
    health.ok(string.format("Terminal detected: %s (PID %d)", name, pid))
  else
    health.warn("Could not locate a supported terminal in the process tree", { "hyprfade will still work when called manually with a specific opacity", "Only HyprfadeToggle and HyprfadeReset auto-detection will be affected" })
  end
end
